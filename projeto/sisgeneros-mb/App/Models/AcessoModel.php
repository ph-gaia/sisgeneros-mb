...
class AcessoModel extends CRUD
{
    ...
    private function evitarDuplicidade()
    {
        ...
        $stmt = $this->pdo->prepare("SELECT * FROM {$this->entidade} WHERE id != ? AND nip = ?");
        $stmt->bindValue(1, $this->getId());
        $stmt->bindValue(2, $this->getNip());
        $stmt->execute();
        if ($stmt->fetch(\PDO::FETCH_ASSOC)) {
            msg::showMsg('Já existe um registro com este NIP.'
                . '<script>focusOn("nip")</script>', 'warning');
        }
    }
    ...
    private function buildSetters()
    {
        $this->setId(filter_input(INPUT_POST, 'id') ?? time())
            ...
            ->setNip(filter_input(INPUT_POST, 'nip', FILTER_SANITIZE_SPECIAL_CHARS))
    ...
    private function validaNip()
    {
        $value = v::stringType()->notEmpty()->validate($this->getNip());
        if (!$value) {
            msg::showMsg('O campo NIP deve ser preenchido corretamente.'
                . '<script>focusOn("nip");</script>', 'danger');
        }
        return $this;
    }
